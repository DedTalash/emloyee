...
const EmployeeForm: React.FC<Props> = (props: Props) => {
    const emptyEmployee = useCallback(getEmptyEmployee,[])
    ...
    const validateNameHook = useCallback(validateName, [employee]);
    ...
    async function onSubmit(event: any) {
        ...
        const errorMessage = await props.onSubmit(employee);
        if (errorMessage) {
            alert(errorMessage);
